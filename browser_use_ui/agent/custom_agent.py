...
class CustomAgent(Agent):
    def __init__(
        ...
        # Override the API key with the one from the environment
        openai_api_key = os.getenv("OPENAI_API_KEY")
        if openai_api_key and openai_api_key.startswith("sk-proj"):
            # Force the correct API key from the environment
            os.environ["OPENAI_API_KEY"] = openai_api_key
            print(f"Using API key from environment: {openai_api_key[:10]}...")
